...
class Diecast extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      selectDate: moment().add("days", -16).format("yyyy-MM-DD"), //moment().format("yyyy-MM-DD"),
      startDate: moment().add("days", -22).format("yyyy-MM-DD"),
      finishDate: moment().add("days", -16).format("yyyy-MM-DD"),
  ...
  getModel = async () => {
    const array = await httpClient.get(server.DIECASTMODEL_URL);
  ...
  getParameter = async () => {
    const array = await httpClient.get(
      server.DIECASTPARAMETER_URL + "/" + this.state.model
    );
    // console.log(array.data.result);
    const options = array.data.result.map((d) => ({
      label: d.Parameter,
  ...
  getMachine = async () => {
    const array = await httpClient.get(
      server.DIECASTMACHINE_URL + "/" + this.state.model
    );
    const options = array.data.result.map((d) => ({
      label: d.Machine_no,
      value: d.Machine_no,
              ...
              <div className="row mb-2">
                ...
                <div className="col-sm-6">
                  <ol className="breadcrumb float-sm-right">
                    ...
                    <li className="breadcrumb-item active">Die-Cast</li>
                  </ol>
                </div>
              </div>
            </div>
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            Swal.fire({
                              ...
                              if (this.state.dataday.length > 0) {
                                for (
                                  let i = 0;
                                  i < this.state.dataday.length;
                                  i++
                                ) {
                                  if (this.state.dataday[i].AVG !== null) {
                                    Swal.fire({
                                      icon: "success",
                                      title: "Success",
                                      type: "success",

...
class MOTORDIM extends Component {
  ...
  getModel = async () => {
    try {
      // Assuming you have startDate and finishDate in your component state
      const startDate = this.state.startDate;
      const finishDate = this.state.finishDate;
  
      // Make a GET request with the startDate and finishDate parameters
      const array = await httpClient.get( `${server.MOTORDIMMODEL_URL}/${startDate}/${finishDate}`);
  ...
  getProductionline = async () => {
    try {
      const { model, startDate, finishDate } = this.state;
  
      // Make a GET request with the model, startDate, and finishDate parameters
      const array = await httpClient.get(
        `${server.MOTORDIMLINE_URL}/${model}/${startDate}/${finishDate}`
      );
  ...
  getMachine = async () => {
    try {
      const { productionline, model, startDate, finishDate } = this.state;
  
      // Make a GET request with the production line, model, startDate, and finishDate parameters
      const array = await httpClient.get(
        `${server.MOTORDIMMACHINE_URL}/${productionline[0].label}/${model}/${startDate}/${finishDate}`
      );
